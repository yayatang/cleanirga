...
interpolate_std <- function(samples) { # samples should be "samp" from import_sample function
  ...
  standards <- select(samples, std_time_int, std_integral) %>%
    ...
    ) %>% # groups of standards have the same number
...
#'
#' @param interpolate_data This is the standard
#' @inheritParams interpolate_std
#' @inheritParams import_sample
#'
#'
combine_all <- function(samples, interpolate_data, meta_data) { ## samples should be "samp" from import_sample function
  ...
  # tomo_idx_flush <- (as.POSIXlt(samp_trim$time_flush, format='%H:%M')$hour < 6)
  # tomo_idx_msre <- (as.POSIXlt(samp_trim$time_msre, format='%H:%M')$hour < 6)
  #
  # # change day to the next day bc it was after 12am
  # samp_trim$day_flush[tomo_idx_flush==T] <- as.character(as.Date(meta$day_flush)+1)
  # samp_trim$day_msre[tomo_idx_msre==T] <- as.character(as.Date(meta$day_msre)+1)
  # #-----end next day interlude----------
  #
  # format into date data type
...
plot_trt_daily <- function(treatment, data_sheet) {
  trt_ref <- data.frame(
    "abb" = c("R", "C", "G", "ME", "MR", "MA", "WA", "WE", "WR", "WW", "WS", "WN"),
    "name" = c(
      "Reference", "Control", "Whole_prey", "Mantis_Excreta",
...
plot_trt_cumul <- function(treatment, data_sheet) {
  ...
  ggsave(paste0("results/phase2_4_cumul_C_per_treatment_", trt_string, ".pdf"), width = 8, height = 6, dpi = 400)
}
